#!/usr/bin/env python3
"""
Validação visual final - captura e salva imagens para verificação humana.
Salva: frame original, overlay renderizado, composição final.
"""
...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "visual_validation")
...
class OverlayWindow:
    ...
    def create(self):
        ...
        user32.SetLayeredWindowAttributes(self.hwnd, 0x000000, 0, 0x00000001)  # LWA_COLORKEY
        user32.ShowWindow(self.hwnd, 5)
        user32.UpdateWindow(self.hwnd)

        self.hdc_window = user32.GetDC(self.hwnd)
    ...
    def present(self):
        ...
        ctypes.windll.gdi32.BitBlt(self.hdc_window, 0, 0, self.width, self.height, self.hdc_mem, 0, 0, 0x00CC0020)

    def destroy(self):
        user32 = ctypes.windll.user32
        gdi32 = ctypes.windll.gdi32
        if self.hbm_old and self.hdc_mem:
            gdi32.SelectObject(self.hdc_mem, self.hbm_old)
        if self.hdc_mem:
...
def main():
    print("=" * 70)
    print("VALIDAÇÃO VISUAL FINAL - Captura de Imagens para Verificação")
    print("=" * 70)
    ...
    with mss.mss() as sct:
        ...
        while time.time() - start < 3:
            ...
            # 1. Captura frame original (MSS)
            img = sct.grab(monitor)
            frame_original = np.array(img)
            frame_bgr = cv2.cvtColor(frame_original, cv2.COLOR_BGRA2BGR)
            ...
            overlay.draw_diagnostic(frame_bgr, board, pieces, fps)
            overlay.present()
            
            # A cada 10 frames, salva as 3 imagens
            if frame_count % 10 == 0 and saved_count < 5:
                # 1. Frame original capturado pelo MSS
                orig_path = os.path.join(OUTPUT_DIR, f"frame_{saved_count:02d}_original.png")
                cv2.imwrite(orig_path, frame_bgr)
                
                # 2. Frame com overlay renderizado (buffer do overlay)
                overlay_path = os.path.join(OUTPUT_DIR, f"frame_{saved_count:02d}_overlay.png")
                # Converte BGRA para BGR para salvar
                overlay_bgr = cv2.cvtColor(overlay.img_buffer, cv2.COLOR_BGRA2BGR)
                cv2.imwrite(overlay_path, overlay_bgr)
                
                # 3. Composição final: frame original + overlay (simula o que o olho vê)
                # Como o overlay usa color key (preto=transparente), compomos manualmente
                # Frame original + overlay (onde overlay != preto)
                overlay_bgra = overlay.img_buffer
                mask = np.any(overlay_bgra[:, :, :3] != 0, axis=2)  # pixels não-pretos no overlay
                composition = frame_bgr.copy()
                composition[mask] = overlay_bgra[mask, :3]
                
                comp_path = os.path.join(OUTPUT_DIR, f"frame_{saved_count:02d}_composicao.png")
                cv2.imwrite(comp_path, composition)
                
                print(f"  Salvos: frame_{saved_count:02d}_original.png, _overlay.png, _composicao.png")
                saved_count += 1
